...
$attributes = array('class' => 'form-horizontal',
 ...
 echo form_open ('admin_Acquisti/modificaRecord/'.$record->id,$attributes);
...
<div class='row'>
 <div class='col-md-12'>
   <label class='col-md-12 col-xs-12  control-label'>Fattura*</label>
   <div class='col-md-12 col-xs-12'>
   <?php

         if(empty(set_value('fattura'))){
           $val=($record->fattura) ;
         }else{
           $val=set_value('fattura') ;
         }

    ?>
 <input class='form-control' type='text' value='<?=$val?>'   id='id_class_fattura'   name='fattura' maxlength='400'  >
   </div>
</div>
</div>
...
function controlla_e_invia(){
    ...
    if(id==''){
          ...
          var message = '<?=lang('fattura_not_empty')?>' ;
          $('#alertModal').find('.modal-body p').text(message);
          $('#alertModal').modal('show');
          return false;
